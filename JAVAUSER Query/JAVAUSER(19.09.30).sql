...
SELECT SB.s_num, SB.s_name, COUNT(ST.sd_num) FROM subject SB INNER JOIN student ST 
ON SB.s_num = ST.s_num GROUP BY SB.s_name ORDER BY SB.s_num;
--3. 전체 학과명에 소속된 학생 수를 출력하기
SELECT SB.s_num, SB.s_name, COUNT(ST.sd_num) FROM subject SB LEFT OUTER JOIN student ST 
ON SB.s_num = ST.s_num GROUP BY SB.s_num, SB.s_name ORDER BY SB.s_num;
